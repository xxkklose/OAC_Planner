...
public:
    ...
    Path();
    ~Path();
    void updatePath(const Path& path_in)
    {
        
        nodes_.clear();
        for(int i=0; i<path_in.nodes_.size(); i++)
        {
            Node* temp_node = new Node(*(path_in.nodes_[i]));
...
public: 
    ...
    Plane(const Eigen::Vector3d &p_surface, World* world);
};
...
public:
    ...
    World(const float &resolution=0.1f, const ros::NodeHandle& nh=ros::NodeHandle("~"));
    ~World();
     ...
     * @return void
     */
    void initGridMap(grid_map::GridMap& gridMap);
    ...
    bool project2GridMapSurf(const float &x,const float &y,Eigen::Vector3d* p_surface);
