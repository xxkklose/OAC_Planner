...
void ElevationMapping::setupSubscribers() {  // Handle deprecated point_cloud_topic and input_sources configuration.
  ...
  const bool configuredInputSources = inputSources_.configureFromRos("input_sources"); // 自己定义的一个输入管理源，从config文件中读取配置
  const bool hasDeprecatedPointcloudTopic = nodeHandle_.hasParam("point_cloud_topic"); // 检测是否直接定义了point_cloud_topic
  
  // 根据point_cloud_topic和input_sources的配置情况，决定订阅point_cloud_topic的方式
...
bool ElevationMapping::readParameters(bool reload) {
  ...
  nodeHandle_.param("point_cloud_topic", parameters.pointCloudTopic_, std::string("/points")); // 默认的读取方式，推荐使用input_sources
...
void ElevationMapping::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& pointCloudMsg, bool publishPointCloud,
  ...
  if (!receivedFirstMatchingPointcloudAndPose_) {
    ...
    ROS_WARN("oldestPoseTime: %f, currentPointCloudTime: %f", oldestPoseTime, currentPointCloudTime);
  ...
  if (!parameters.ignoreRobotMotionUpdates_) {
    ...
    if (!poseMessage) {
      ...
      ROS_WARN("robotPoseTime: %f, lastPointCloudUpdateTime: %f", robotPoseCache_.getOldestTime().toSec(), lastPointCloudUpdateTime_.toSec());
      if (robotPoseCache_.getOldestTime().toNSec() > lastPointCloudUpdateTime_.toNSec()) {
        ROS_ERROR("The oldest pose available is at %ld, requested pose at %ld", robotPoseCache_.getOldestTime().toNSec(),
                  lastPointCloudUpdateTime_.toNSec());
      } else {
